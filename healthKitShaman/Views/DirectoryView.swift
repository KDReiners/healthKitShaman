            ...
            List
            {
                ...
                DisclosureGroup("AddendumTypes", isExpanded: $isExpanded) {
                    ForEach(0..<addendumTypeViewModel.items.count, id: \.self) { i in
                        NavigationLink(addendumTypeViewModel.items[i].name ?? "", destination: DataNavigatorView(masterObject: addendumTypeViewModel, detailObject: AddendumTimeCapsules(resolution: 3600), recordIndex: i, childrenRelationKey: "name"), tag: addendumTypeViewModel.items[i], selection: $select)
